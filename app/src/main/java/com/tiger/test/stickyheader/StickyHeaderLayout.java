...
//                LogUtils.d("onScrolled: dy--> " + dy);
    ...
    private void addStickyLayout() {
        ...
        FrameLayout.LayoutParams lp = new LayoutParams(FrameLayout.LayoutParams.MATCH_PARENT, FrameLayout.LayoutParams.WRAP_CONTENT);
    ...
    private void updateStickyView(int dy) {
        ...
        if (adapter != null) {
            if (mStickyLayout.getChildCount() == 0) {
                stickyItemHolder = adapter.onCreateViewHolder(mStickyLayout, TYPE_STICKY_LAYOUT);
                mStickyLayout.addView(stickyItemHolder.itemView);
            }
...
//                LogUtils.d("updateStickyView: --------------------visible");
                if (onStickItemStateChangeListener != null) {
                    onStickItemStateChangeListener.onStickItemStateChanged(stickyItemHolder, true);
                }
            }
    ...
    public interface OnStickItemStateChangeListener {
        void onStickItemStateChanged(RecyclerView.ViewHolder holder, boolean visible);
    }
}